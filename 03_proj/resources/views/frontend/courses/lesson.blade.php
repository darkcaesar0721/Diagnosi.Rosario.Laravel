           ...
           {{-- Progress Bar --}}
            ...
            <div class="row w-100  justify-content-md-center mb-5">
                <div class="b_progress_container">
                    <div class="b_progress_bar gradient-bg" style="width:{{$percent}}%; background: {{$lesson->color1}} !important;">
                        
                    </div>
                    <div class="b_progress gradient-bg" style="background: {{$lesson->color1}}">
                        {{ $percent }}%
                    </div>
                </div>
            </div>
...
@push('after-scripts')
    <script>
        {{--var default_font = '{{$lesson->color1}}';--}}
        {{--var default_font_size = '{{$lesson->color2}}';--}}
        {{--var default_font_color = '{{$lesson->default_font_color}}';--}}

        var color1 = '{{$lesson->color1}}';
        var color2 = '{{$lesson->color2}}';
        ...
        $(function() {
            ...
            // $('.container-fluid p, label, b, th, tr').attr('style', "color: " + default_font_color + " !important;");

            // $(".q_number").attr('style', 'background: ' + color1 + ' !important');
        });
        ...
        $(function() {
            // $('.container-fluid').attr('style', "font-family: " + default_font + " !important;");
            // $('.container-fluid p, label, b, th, tr').attr('style', "color: " + default_font_color + " !important;");
            // $('.container-fluid').attr('style', "color: " + default_font_color + " !important");
        ...
        $(".rating-stars .star").click(function(event){
            if($('#percent').val() == 1000 && $('#reported').val() == 10){
                ...
                $(this).closest('.rating-stars').find('.star i').css('color',color1);
                $(this).closest('.rating-stars').find('.starinput').val(value);
                var color = $(this).closest('.rating-stars').find('.star_color').val();
                for(i=1;i<=value;i++){
                    $(this).closest('.rating-stars').find('.star-'+i+' i').css('color',color2);
                }
            }
        });
        $(".rate").click(function(){
            if($('#percent').val() == 1000 && $('#reported').val() == 10){
                ...
                var value = $(this).data('value');
                $(this).closest('.rating-box').find('.rate').css('background',color1);
                var color = $(this).closest('.rating-box').find('.star_color').val() ?? color2;
