<div class='rating-box rating_content'  data-required="@if($question->required=="1") 1 @else 0 @endif" >
                ...
                @foreach($content as $c)
                    @if(isset($c->label))
                        <div class="{{isset($col)?$col:''}} mt-1">
                            <li class='rate rate-{{$i}}' data-q_id="{{$question->id}}" name="rating" title="{{$c->score}}" data-value='{{$i}}' data-score="{{$c->score}}" style="background-color: {{$lesson->color1}}">
                                {{$c->label}}
                            </li>
                        </div>
    ...
    <input type="hidden" name="star_color" class="star_color" value="{{$lesson->color2}}">
